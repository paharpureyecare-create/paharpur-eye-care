...
export const RetailSalesView: React.FC = () => {
  ...
  return (
    <div className="space-y-6 pb-12">
      ...
      {/* Retail Sales Table & Mobile Cards */}
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
        
        {/* Mobile Sales Cards (Android Touch-Optimized) */}
        <div className="md:hidden divide-y divide-slate-100">
          {filtered.length === 0 ? (
            ...
            filtered.map((sale: any) => {
              ...
              return (
                <div key={invNumber} className="p-3.5 space-y-2.5 hover:bg-slate-50 transition-colors">
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      <div className="font-mono text-xs font-bold text-teal-900 bg-teal-50 border border-teal-200 px-2 py-0.5 rounded w-fit">
                        {invNumber}
                      </div>
                      ...
                      <div className="text-xs text-slate-500">
                        {patientMrd ? `MRD: ${patientMrd} • ` : ''}{sale.mobile || 'Walk-in'}
                      </div>
                    </div>
                    <div className="text-right">
                      <div className="text-base font-black text-slate-900">₹{totalAmt}</div>
                    ...
                    {itemsList.slice(0, 2).map((it: any, idx: number) => (
                      <div key={idx} className="flex justify-between text-slate-700">
                        ...
                        <span className="font-semibold text-slate-900">
                          {it.quantity ?? it.qty ?? 1}x @ ₹{it.unitPrice ?? it.rate ?? 0}
                        </span>
                      </div>
                    ))}
                    {itemsList.length > 2 && (
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <div className="flex items-center gap-2">
                      <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-[10px] font-bold bg-slate-100 text-slate-800 border border-slate-200">
                        {pMode}
                      </span>
                      {sale.due > 0 ? (
                    ...
                    {hasPermission('Sales', 'print') && (
                      <button
                        ...
                        className="min-h-[38px] px-3 bg-teal-50 hover:bg-teal-100 text-teal-800 border border-teal-200 rounded-xl text-xs font-bold flex items-center gap-1.5 active:scale-95 transition cursor-pointer"
                      >
